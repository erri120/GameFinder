 ...
 */
function getAppFoldersFromDrive(drivePath: string): Result<string[], GameFinderError[]> {
  const paths: string[] = [];
  const errors: GameFinderError[] = [];

  // Check for ModifiableWindowsApps directory
  const modifiableWindowsAppsPath = join(drivePath, 'Program Files', 'ModifiableWindowsApps');
