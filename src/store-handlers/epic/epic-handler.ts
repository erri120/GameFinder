...
function getDefaultManifestDirectory(): string {
  ...
  switch (platform()) {
    case 'win32': {
      ...
      return join(localAppData, 'Epic', 'EpicGamesLauncher', 'Data', 'Manifests');
    }
    ...
    case 'linux':
      ...
      return join(home, '.config', 'Epic', 'EpicGamesLauncher', 'Data', 'Manifests');
 ...
 */
function parseManifestFile(filePath: string): Result<EGSManifestFile, GameFinderError> {
  try {
    const content = readFileSync(filePath, 'utf8');
    const manifest = JSON.parse(content) as Partial<EGSManifestFile>;

    // Validate required fields
    if (typeof manifest.CatalogItemId !== 'string' || manifest.CatalogItemId === '') {
      return err({
