 ...
 */
function parseInstalledDepots(depotsObj: VdfObject | undefined): InstalledDepot[] {
...
export function parseAppManifest(filePath: string): Result<AppManifest, GameFinderError> {
  ...
  const installationDirectory = join(steamAppsDir, 'common', installDirResult.value);
  ...
  const installedDepots = parseInstalledDepots(getObject(appState, 'InstalledDepots'));
  ...
  return ok({
    ...
    sizeOnDisk,
    buildId: buildIdNum,
    lastOwner: lastOwner !== 0n ? lastOwner : undefined,
    bytesToDownload,
    bytesDownloaded,
    bytesToStage,
    bytesStaged,
